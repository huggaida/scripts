...
class OE_OT_OffsetEdges(bpy.types.Operator, OffsetBase):
    """Offset Edges."""
    ...
    def change_depth_mode(self, context):
        if self.depth_mode == 'angle':
            self.width, self.angle = OE_OT_OffsetEdges.depth_to_angle(self.width, self.depth)
        else:
            self.width, self.depth = OE_OT_OffsetEdges.angle_to_depth(self.width, self.angle)
...
class OEP_OT_OffsetEdgesProfile(bpy.types.Operator, OffsetBase):
    ...
    def analize_profile(context, ob_profile, resolution):
        ...
        me = ob_profile.to_mesh(depsgraph=context.depsgraph, apply_modifiers=False)
...
class OEM_MT_OffsetEdgesMenu(bpy.types.Menu):
...
classes = (
OffsetEdgesPreferences,
OE_OT_OffsetEdges,
OEP_OT_OffsetEdgesProfile,
OEM_MT_OffsetEdgesMenu,
VIEW3D_PT_OffsetEdges
) 
