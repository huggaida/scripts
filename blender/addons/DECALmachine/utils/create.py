...
def get_info_decal_geometry(context, scene, sel, debug=False):
    ...
    for obj in sel:
        source = bpy.data.objects.new(name=obj.name, object_data=obj.to_mesh(context.depsgraph, apply_modifiers=True))
...
def get_decal_source_objects(context, scene, sel, active, debug=False):
    ...
    # NOTE: since b936d7b16c6 and after a4869df4c9d2 the mods need to be applied, while still in the old scene!
    # if you switch to the bakescene first and then attempt to apply the mods, the mods, nothing will happen

    for obj in sel:
        source = obj.copy()
        source.data = obj.to_mesh(context.depsgraph, apply_modifiers=True)
...
def create_decal_geometry(context, scene, coords, height):
    ...
    # it's weird, but only by translating will the decal decal update properly, obj.data.update() and scene.update() have no effect
