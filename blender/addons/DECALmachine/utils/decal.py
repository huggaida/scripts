...
import os
import re
from . material import get_decal_textures, get_decalgroup_from_decalmat, get_parallaxgroup_from_decalmat, get_heightgroup_from_parallaxgroup, get_decal_texture_nodes, auto_match_material, remove_decalmat
from . raycast import cast_bvh_ray_from_object, get_origin_from_object, find_nearest, get_origin_from_face, get_origin_from_object_boundingbox, cast_bvh_ray_from_mouse, get_grid_intersection
...
from . math import create_rotation_matrix_from_normal
...
def align_decal(decalobj, scene, force_cursor_align=False):
    """
    ...
    # set location and rotation via raycast from the mouse
        ...
        # for performance, do it in 2 passes, first all visible objects, without flattening, FWIW, Blender's cursor seems to do something similar
        hitobj, loc, normal, _, _ = cast_bvh_ray_from_mouse(mousepos, flatten_visible=False, exclude_decals=True, debug=False)

        # then a second ray to the flattened best hit of the first object
        if hitobj:
            hitobj, loc, normal, _, _ = cast_bvh_ray_from_mouse(mousepos, candidates=[hitobj], flatten_visible=True, exclude_decals=False, debug=False)
...
def apply_decal(decalobj, target=None, update_scene=None, raycast=False, force_automatch=False):
    """
    ...
    # update the scene, or the raycast may fail to hit what should be hit, as in the case of InsertDecal() where the decal is moved by code before the raycast
    if update_scene:
        update_scene.update()
...
def create_panel_uvs(bm, geo_sequences, panel, width=None):
    ...
    for geo, cyclic in geo_sequences:
        ...
        for gidx, (face, edge) in enumerate(geo):
            ...
            distance = (midpoint - midpoint_next).length * 1 / width
            u_end += distance
