...
def get_ray_distance_from_verts(target, source, ray_direction, backtrack=None, limit=None, debug=False):
...
def cast_bvh_ray_from_mouse(mousepos, candidates=None, flatten_visible=False, exclude_decals=True, debug=False):
    ...
    if exclude_decals:
        visible = [(obj, None) for obj in candidates if obj.type == "MESH" and not obj.DM.isdecal]
    else:
        visible = [(obj, None) for obj in candidates if obj.type == "MESH"]

    # flatten visible objects (duplicates)
    if flatten_visible:
        objects = []
    ...
    # remove flattened object duplicates
    if flatten_visible:
        for obj, src in objects:
...
def find_nearest(targets, origin, debug=False):
    ...
    if debug:
        print("best hit:", target, location, normal, index, distance)
