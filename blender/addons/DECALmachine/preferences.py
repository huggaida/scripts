...
class DECALmachinePreferences(bpy.types.AddonPreferences):
    ...
    def draw_machin3(self, layout):
        ...
        row = column.split(factor=0.3)
        row.scale_y = 1.2
        row.label(text="MESHmachine", icon_value=installed if get_addon('MESHmachine')[1] else missing)
        r = row.split(factor=0.2)
        ...
        row = column.split(factor=0.3)
        row.scale_y = 1.2
        row.label(text="MACHIN3tools", icon_value=installed if get_addon('MACHIN3tools')[1] else missing)
        r = row.split(factor=0.2)
        ...
        # row.operator("wm.url_open", text="Report Errors", icon='ERROR').url = "https://machin3.io/DECALmachine/docs/faq/#reporting-errors-or-problems"
        ...
        row.operator("wm.url_open", text="polycount", icon="COMMUNITY").url = "https://polycount.com/discussion/186089/blender-decal-machine-a-quick-way-to-create-decals-for-hard-surface/p1"
    ...
    def draw_recommended(self, layout):
        ...
        row = column.split(factor=0.2)
        row.scale_y = 1.2
        row.label(text="Group Pro", icon_value=installed if get_addon("Group Pro")[1] else missing)
        ...
        row = column.split(factor=0.2)
        row.scale_y = 1.2
        row.label(text="Batch Operations™", icon_value=installed if get_addon("Batch Operations™")[1] else missing)
        rs = row.split(factor=0.3)
        r = rs.row(align=True)
        ...
        row = column.split(factor=0.2)
        row.scale_y = 1.2
        row.label(text="Hard Ops 9", icon_value=installed if get_addon('Hard Ops 9')[1] else missing)
        rs = row.split(factor=0.3)
        r = rs.row(align=True)
        ...
        row = column.split(factor=0.2)
        row.scale_y = 1.2
        row.label(text="BoxCutter", icon_value=installed if get_addon('BoxCutter')[1] else missing)
        rs = row.split(factor=0.3)
        r = rs.row(align=True)
        r.operator("wm.url_open", text="Web", icon='URL').url = "https://masterxeon1001.com"
        r.operator("wm.url_open", text="Twitter", icon='URL').url = "https://twitter.com/mxeon1001"
        ...
        row = column.split(factor=0.4)
        row.scale_y = 1.2
        row.label(text="Piotr Jedziński: 100+ Sci-fi Decals")
        rs = row.split(factor=0.45)
        r = rs.row(align=True)
        r.operator("wm.url_open", text="Twitter", icon='URL').url = "https://twitter.com/radi0n"
        rs.operator("wm.url_open", text="Gumroad", icon='URL').url = "https://gumroad.com/l/NIAlc"
