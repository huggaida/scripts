...
from .. utils.raycast import get_ray_distance_from_verts
...
class Project(bpy.types.Operator):
    ...
    def invoke_modal(self, context, event, decals):
        ...
        for decal in decals:
            ...
            if target:
                ...
                projected.data = target.to_mesh(context.depsgraph, apply_modifiers=True)
                projected.modifiers.clear()
                projected.name = decal.name + "_projected"

                # determine  base projection depth
                front, back = get_ray_distance_from_verts(projected, decal, (0, 0, -1), 0.1)
    ...
    def project(self, context, event, decal, target, projected=None, depth=None):
        ...
        # add thickness to decal, based on fron and backsiade distances, with proper offset. compensate for decal scaling and add 20% extra for good measure
        ...
        # apply the decalmat
        projected.data.materials.clear()
