...
from .. utils.decal import set_props_and_node_names_of_library_decal, set_decalobj_props_from_material, set_decalobj_name, set_defaults, get_panel_width, create_panel_uvs, change_panel_width, sort_panel_geometry
...
class Slice(bpy.types.Operator):
    ...
    def slice(self, context, event, target, cutter):
        ...
        panel = target.copy()
        panel.data = target.to_mesh(context.depsgraph, apply_modifiers=True)
        ...
        panel = self.topo_slice(context, mcol, panel, cutter) if event.alt else self.float_slice(context, event, panel, cutter)
        ...
        if panel:
            # finsh the panel
            self.finish_panel_decal(context, panel, target, cutter)
    ...
    def float_slice(self, context, event, panel, cutter):
        """
        ...
        def create_float_slice_geometry(bm, sequences, normals, width, debug=False):
            geo_sequences = []
            ...
            for seq, cyclic in sequences:
                ...
                for idx, v in enumerate(seq):
                    ...
                    if debug:
                        draw_vector(normal * 0.1, origin=panel.matrix_world @ v.co, color=(1, 0, 0))
                        draw_vector(direction * 0.05, origin=panel.matrix_world @ v.co, color=(0, 0, 1))
                        draw_vector(cross * 0.05, origin=panel.matrix_world @ v.co, color=(0, 1, 0))
        ...
        # intersect and apply the boolean mod
        intersect(panel, cutter)
        flatten(panel)
        ...
        normals, bmt = find_nearest_normals(bm, flat_target)
        ...
        # create panel faces
        geo = create_float_slice_geometry(bm, sequences, normals, width=width)
    ...
    def finish_panel_decal(self, context, panel, target, cutter):
        dm = context.scene.DM
