...
class Create(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if dm.create_decaltype == 'INFO':
            ...
            # switch to bake scen
            # NOTE: since b936d7b16c6 and after a4869df4c9d2, the scene change needs to be done
            # AFTER the mods of the source objects are applied!
            context.window.scene = bakescene
    ...
    def insert_decal(self, context, decalpath, decaltype, location, width):
        ...
        # align the decal to the cursor and scale it
            ...
            align_decal(decalobj, context.scene, force_cursor_align=True)
        ...
        if decalmat:
            ...
            if not baked:
                ...
                apply_decal(decalobj, update_scene=context.scene, raycast=True)
